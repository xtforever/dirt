#ifndef lint
static char rcsid[] =
   "$Id: xscan_create.c,v 1.2 91/12/17 12:00:21 rlh2 Rel $";
#endif /* !lint */
...
String *app_defaults;
{
	...
	for (i = 0; i < XtNumber(WI_xscan_classes); i++)
		WcRegisterClassPtr(new_context,
			WI_xscan_classes[i].class_name,
			*(WI_xscan_classes[i].class));

	register_names(new_context, WI_xscan_action_names,
			XtNumber(WI_xscan_action_names), TRUE);
	return (dpy);
}
...
String app_defaults[];
{
	...
	dpy = XukcToolkitInitialize(dpy_str, "xscan", "XScan",
					argc, argv, options, num_options,
					app_defaults);
	new_shell = XtAppCreateShell("xscan", "XScan",
...
Widget toplevel_shell;
{
	...
	WI_xscan = toplevel_shell;

	/* create all the widgets in the user interface */
	WcWidgetCreation(toplevel_shell);

	/* bind the widget ids. to variables */
	for (i = 0; i < XtNumber(WI_xscan_bind_list); i++) {
		*(WI_xscan_bind_list[i].w) = WcFullNameToWidget(toplevel_shell,
			WI_xscan_bind_list[i].str);
		if (*(WI_xscan_bind_list[i].w) == NULL) {
			printf("WI_xscan: failed to bind user interface.  Missing defaults file?\n");
			exit(1);
		}
	}

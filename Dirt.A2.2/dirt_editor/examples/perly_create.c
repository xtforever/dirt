#ifndef lint
static char rcsid[] =
   "$Id: perly_create.c,v 1.3 91/12/17 12:00:16 rlh2 Rel $";
#endif /* !lint */
...
String *app_defaults;
{
	...
	for (i = 0; i < XtNumber(WI_perly_classes); i++)
		WcRegisterClassPtr(new_context,
			WI_perly_classes[i].class_name,
			*(WI_perly_classes[i].class));

	WcAllowDuplicateConstructorReg(TRUE);
	for (i = 0; i < XtNumber(WI_perly_constructors); i++)
		WcRegisterConstructor(new_context,
			WI_perly_constructors[i].constructor_name,
			WI_perly_constructors[i].constructor);

	register_names(new_context, WI_perly_action_names,
			XtNumber(WI_perly_action_names), TRUE);
	return (dpy);
}
...
String app_defaults[];
{
	...
	dpy = XukcToolkitInitialize(dpy_str, "perly", "Perly",
					argc, argv, options, num_options,
					app_defaults);
	new_shell = XtAppCreateShell("perly", "Perly",
...
{
	create_connections(WI_perly_widget_connections,
			XtNumber(WI_perly_widget_connections));
}
...
Widget toplevel_shell;
{
	...
	WI_perly = toplevel_shell;

	/* create all the widgets in the user interface */
	WcWidgetCreation(toplevel_shell);

	/* bind the widget ids. to variables */
	for (i = 0; i < XtNumber(WI_perly_bind_list); i++) {
		*(WI_perly_bind_list[i].w) = WcFullNameToWidget(toplevel_shell,
			WI_perly_bind_list[i].str);
		if (*(WI_perly_bind_list[i].w) == NULL) {
			printf("WI_perly: failed to bind user interface.  Missing defaults file?\n");
			exit(1);
		}
	}
